      ...
      <header className="header-container">
        ...
        <div className="brand-group" style={{ cursor: 'pointer' }}>
          ...
          <div>
            ...
            <div className="brand-badge">
              <span className="dot dot-active"></span> {t('brand_subtitle')}
            </div>
          </div>
        </div>
        ...
        <div className="telemetry-bar">
          <div className="telemetry-item" style={{ background: 'rgba(16, 185, 129, 0.15)', padding: '0.25rem 0.6rem', borderRadius: '6px', border: '1px solid rgba(16, 185, 129, 0.4)' }}>
            ...
            <span className="telemetry-label" style={{ color: '#6ee7b7', fontWeight: 600 }}>{t('balance')}:</span>
          ...
          <div className="telemetry-item" style={{ background: 'rgba(56, 189, 248, 0.12)', padding: '0.25rem 0.6rem', borderRadius: '6px', border: '1px solid rgba(56, 189, 248, 0.35)' }}>
            ...
            <span className="telemetry-label" style={{ color: '#7dd3fc', fontWeight: 600 }}>{t('grid_power')}:</span>
            <span className="telemetry-value" style={{ color: totalGridEnergy > 0 ? '#38bdf8' : '#f87171', fontSize: '0.9rem', fontFamily: 'Fira Code, monospace' }}>
              {activePowerPlants.length > 0 ? `${totalGridEnergy.toLocaleString()} / ${maxGridEnergy.toLocaleString()} kWh` : t('no_power_plant')}
            </span>
          </div>
          ...
          <div className="telemetry-item" style={{ background: 'rgba(6, 182, 212, 0.12)', padding: '0.25rem 0.5rem', borderRadius: '6px', border: '1px solid rgba(6, 182, 212, 0.3)' }}>
            ...
            <span className="telemetry-label">{t('map')}:</span>
            <select
              value={currentBiome}
          ...
          <button
            onClick={() => {
              const seed = prompt(t('seed_prompt'), `SEED_${Math.floor(Math.random() * 90000 + 10000)}`);
        ...
        <div className="control-group">
          <div className="speed-selector">
            ...
            <button
              onClick={() => setTickRate(1000)}
              className={`speed-btn ${tickRate === 1000 ? 'active' : ''}`}
            >
              1x
            </button>
            <button
              onClick={() => setTickRate(500)}
              className={`speed-btn ${tickRate === 500 ? 'active' : ''}`}
            >
              2x
            </button>
            <button
              onClick={() => setTickRate(200)}
              className={`speed-btn ${tickRate === 200 ? 'active' : ''}`}
            >
              5x
            </button>
          </div>
          ...
          <button
            ...
            style={{ padding: '0.35rem 0.55rem', fontSize: '0.72rem', fontWeight: 800, letterSpacing: '0.5px' }}
          >
